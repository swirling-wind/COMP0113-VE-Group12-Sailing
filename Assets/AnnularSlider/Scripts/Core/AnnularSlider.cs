...
public class AnnularSlider : Selectable, IDragHandler, ICanvasElement
{
    ...
    protected virtual void Update()
    {
        ...
        if (isOwner)
        {
            float angle = (360.0f + 270.0f - NormalizedValue * _maxAngle) % 360.0f;
            wind.transform.eulerAngles = new Vector3(0, angle, 0);
            string direction = AngleToDirection(angle);

            Message m = new Message();
            m.rotation = wind.transform.rotation;
            m.normalizedValue = NormalizedValue;
            m.token = token;
            context.SendJson(m);
        ...
        // Debug.Log("Wind Direction: " + direction); // 0 N
        // Debug.Log("Wind Angle: " + wind.transform.forward); // (0, 0, 1)
        // Debug.Log("Angle: " + NormalizedValue * _maxAngle); // 270
        // Debug.Log("Angle: " + _value); // 0.75
    }
    public void ProcessMessage(ReferenceCountedSceneGraphMessage m)
    {
        var message = m.FromJson<Message>();
        wind.transform.rotation = message.rotation;
    ...
    // function for converting the rotation angle into a cardinal or intercardinal direction string
